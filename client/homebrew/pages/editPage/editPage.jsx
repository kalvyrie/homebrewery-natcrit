...
const EditPage = (props)=>{
	...
	const exportDraft = ()=>{
		let fileName = sanitizeFilename(`HB - ${currentBrew.title}`).replaceAll(' ', '');
		if(!fileName || !fileName.length) fileName = 'HB - Untitled-Brew';

		const blob = new Blob([currentBrew.text], { type: 'text/plain' });
	...
	const handleImportFile = (e)=>{
		...
		reader.onload = (e)=>{
			setCurrentBrew((prevBrew)=>({ ...prevBrew, text: e.target.result }));
		};
		reader.readAsText(file);
	};

	const renderLocalFileMenu = ()=>(
		<Nav.dropdown>
			<Nav.item icon='fas fa-file-export'>
				local file
			</Nav.item>
			<Nav.item color='blue' icon='fas fa-download' onClick={exportDraft}>
				export .txt
			</Nav.item>
			<Nav.item
				color='blue'
